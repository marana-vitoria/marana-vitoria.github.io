...
var $indicators = $('<ol>')
  ...
  */
    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      if (correct) {
        ...
        opts = $.extend(opts, {
          title: "Ah... Errou.",
          text: (
            "Voce nao respondeu corretamente...<br/><br/>" +
            "A resposta certa é \"" +
      ...
      if (last_question) {
        opts.confirmButtonText = "Veja seus resuldados";
      }
  ...
  var $social = $("<div>")
    ...
    .html('<div id = "social-text">Gostou do quiz? Compartilhe os resultados com seus amigos, assim eles podem participar também!</div>')
    .appendTo($results_slide);
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      text = "Parabéns! Pontuação perfeita!";
      break;
    case (ratio > 0.9):
      text = "Bom trabalho! Voce acertou a maioria das questoes.";
      break;
    case (ratio > 0.60):
      text = "Muito bom! Digamos que voce passou.";
      break;
    case (ratio > 0.5):
      text = "Bem, pelo menos voce acertou metade das questoes.";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = "Parece que essa foi dificil... Boa sorte da proxima vez.";
      break;
    case (ratio === 0):
      text = "Opa, nenhum acerto... Voce precisa se esforcar mais!";
      break;
  }
  return text;
