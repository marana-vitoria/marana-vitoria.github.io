    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      if (correct) {
        ...
        opts = $.extend(opts, {
          title: "Ah... Errou.",
          text: (
            "Você não respondeu corretamente...<br/><br/>" +
            "A resposta certa é \"" +
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      ...
      break;
    case (ratio > 0.9):
      text = "Bom trabalho! Você acertou a maioria das questões.";
      break;
    case (ratio > 0.60):
      text = "Muito bom! Digamos que você passou.";
      break;
    case (ratio > 0.5):
      text = "Bem, pelo menos você acertou metade das questões.";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = "Parece que essa foi difícil... Boa sorte da próxima vez.";
      break;
    case (ratio === 0):
      text = "Opa, nenhum acerto... Você precisa se esforcar mais!";
      break;
  }
  return text;
